...
export const runtime = 'nodejs'; // ✅ Vercel에서 Node.js 런타임 명시
...
export async function POST(req: Request) {
  try {
    ...
    if (dbError) {
      ...
      return NextResponse.json(
        { error: 'DB 저장 실패', detail: dbError.message },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ error: '서버 에러 발생', detail: err.message }, { status: 500 });
  }
}
